...
- Your definition for the signature of a function you need to convert must be wholly declared in a single line of code and not over multiple lines. The single-line declaration must include the JavaScript `function` keyword. The line must encompass the function's name, the optional `async` prefix if required, and the function's parameters contained in brackets. You cannot use a JavaScript Arrow function to define these 'convertible' functions, although you can use Arrow functions in the code for these 'convertible' functions.
- For any given line of code, you must only invoke one `PRIV_` prefixed function in that line of code. If you attempt to invoke two `PRIV_` prefixed functions from the same line of code, the generated App Services Function will contain syntax errors and result in a deployment error.
